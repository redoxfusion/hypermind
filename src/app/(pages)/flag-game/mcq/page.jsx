...
export default function FlagsGameMCQ() {
  ...
  useEffect(() => {
    async function fetchProgressAndFlags() {
      ...
      try {
        const progressRes = await fetch("/api/user-progress?game=FlagsGameMCQ");
  ...
  useEffect(() => {
    async function fetchTotalScore() {
      try {
        const scoresRes = await fetch("/api/scores?game=FlagsGameMCQ");
        if (scoresRes.ok) {
  ...
  const handleResetProgress = async () => {
    ...
    try {
      ...
      setSelectedOption(null);
      setLoading(true);
      const flagsRes = await fetch("/api/words?level=1&game=FlagsGameMCQ");
      if (!flagsRes.ok) throw new Error("Failed to fetch flags");
